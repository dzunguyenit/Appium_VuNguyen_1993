...
public class AbstractPage {
	...
	// Appium
	protected AndroidDriver<AndroidElement> newServerAppium(DesiredCapabilities capabilities, String url)
			throws MalformedURLException {
		return new AndroidDriver<AndroidElement>(new URL(url), capabilities);
	}

	protected void setBrowserName(DesiredCapabilities capabilities, String browserName, String platform) {
		capabilities.setCapability(browserName, platform);
	}

	protected void setVersion(DesiredCapabilities capabilities, String platformVersion, String version) {
		capabilities.setCapability(platformVersion, version);
	}

	protected void setDevice(DesiredCapabilities capabilities, String deviceName, String device) {
		capabilities.setCapability(deviceName, device);
	}

	protected void setPlatform(DesiredCapabilities capabilities, String platformName, String platform) {
		capabilities.setCapability(platformName, platform);
	}

	protected void setPackage(DesiredCapabilities capabilities, String appPackage, String pathAppPackage) {
		capabilities.setCapability(appPackage, pathAppPackage);
	}

	protected void setActivity(DesiredCapabilities capabilities, String appActivity, String pathAppActivity) {
		capabilities.setCapability(appActivity, pathAppActivity);
	}

	protected void setPathAPKFile(DesiredCapabilities capabilities, String pathFileAPK, String pathAPK) {
		final File classpathRoot = new File(System.getProperty("user.dir"));
		final File appDir = new File(classpathRoot, pathFileAPK);
		final File app = new File(appDir, pathAPK);
		capabilities.setCapability("app", app.getAbsolutePath());
	}

	// Web Browser
	protected void clickOnListElement(WebDriver driver, String locator, int index) {
		List<WebElement> list = driver.findElements(By.xpath(locator));
		list.get(index).click();
	}

	protected void waitForListVisible(WebDriver driver, String locator, int index) {
		try {
			List<WebElement> list = driver.findElements(By.xpath(locator));
			WebDriverWait wait = new WebDriverWait(driver, timeouts);
			wait.until(ExpectedConditions.visibilityOf(list.get(index)));
		} catch (Exception e) {
	...
	protected void quitDriver(AndroidDriver<AndroidElement> driver) {
		driver.quit();
	}
	...
	protected void backToPageAndroid(AndroidDriver<AndroidElement> driver) {
		driver.navigate().back();
	}
	...
	protected void clickToElementAndroid(AndroidDriver<AndroidElement> driver, String locator) {
		try {
			AndroidElement element = driver.findElement(MobileBy.AndroidUIAutomator(locator));
			element.click();
		} catch (Exception e) {
			LogFactory.error("Element is not clickable " + e.getMessage());
		}
	...
	protected void clickToElementAndroidXpath(AndroidDriver<AndroidElement> driver, String locator) {
		try {
	...
	protected void sendKeyToElementAndroid(AndroidDriver<AndroidElement> driver, String locator, String value) {
		try {
			AndroidElement element = driver.findElement(MobileBy.AndroidUIAutomator(locator));
			element.clear();
			element.sendKeys(value);
	...
	protected void waitForControlVisibleAndroid(AndroidDriver<AndroidElement> driver, String locator) {
		try {
			WebDriverWait wait = new WebDriverWait(driver, timeouts);
	...
	protected void waitForControlVisibleAndroidXpath(AndroidDriver<AndroidElement> driver, String locator) {
		try {
			WebDriverWait wait = new WebDriverWait(driver, timeouts);
			By by = By.xpath(locator);
			wait.until(ExpectedConditions.visibilityOfElementLocated(by));
